...
export function TableOfContents({ content, locale }: TableOfContentsProps): React.ReactElement | null {
  ...
  return (
    <nav aria-label={copy.tocTitle} className="tn-surface p-4">
      <div className="mb-3 flex items-center gap-2 text-sm font-bold text-[#101010]">
        <ListIcon className="size-4 text-[#092BB4]" />
        ...
        {items.map((item) => (
          <li key={item.id}>
            <a
              ...
              className={cn(
                'block py-1 text-sm transition-colors hover:text-[#092BB4]',
                item.level === 3 && 'pl-4',
                activeId === item.id ? 'font-semibold text-[#092BB4]' : 'text-[#526071]',
              )}
            >
              {item.text}
            </a>
