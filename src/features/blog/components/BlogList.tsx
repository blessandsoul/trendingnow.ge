    ...
    <div>
      ...
      {topTags.length > 0 && (
        <section aria-labelledby="blog-topics" className="mt-14">
          <h2 id="blog-topics" className="mb-4 text-center text-xl font-black tracking-tight text-[#07152A]">
            {copy.topicsHeading}
          </h2>
          <div className="mb-4 flex flex-wrap justify-center gap-2">
            {topTags.map(({ tag, slug }) => (
              <Link
                ...
                className="rounded-full border border-[#DFE6EF] bg-white px-4 py-1.5 text-sm font-semibold text-[#526071] transition-colors hover:border-[#C89300] hover:text-[#07152A]"
              >
                {tag}
              </Link>
            ))}
      ...
      <div className="text-center">
        <Link
          ...
          className="text-sm font-semibold text-[#526071] underline-offset-4 transition-colors hover:text-[#07152A] hover:underline"
        >
          {copy.viewAllTags}
        </Link>
      </div>
