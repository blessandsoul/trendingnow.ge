...
export function PromoCard({
  ...
  return (
    <article
      className={cn(
        'group relative min-h-[244px] overflow-hidden border border-[#D9DDE7] bg-white shadow-[0_12px_34px_rgba(17,20,27,0.06)]',
        className,
      )}
    >
      <SafeImage
      ...
      <div className="absolute left-0 top-0 h-16 w-1.5 -skew-x-[18deg] bg-[#092BB4]" aria-hidden="true" />
      <div className="relative z-10 flex min-h-[244px] max-w-[84%] flex-col justify-center px-5 py-6 sm:max-w-[56%] sm:px-8">
        <p className="text-xs font-bold uppercase tracking-[0.11em] text-[#061E81]">{eyebrow}</p>
        <h3 className="mt-2 text-xl font-bold leading-7 tracking-[-0.03em] text-[#101010] text-balance sm:text-2xl">
          {title}
        </h3>
        {displayCtaLabel && ctaHref && (
          <Button
            asChild
            variant="outline"
            className="mt-5 h-9 w-fit rounded-[8px] border-[#C8CFD9] bg-white px-4 text-xs font-bold text-[#101010] hover:border-[#092BB4] hover:bg-white hover:text-[#061E81]"
          >
