...
const statusBadgeClasses: Record<StorefrontOrderStatus, string> = {
  PENDING: 'border-[#D9DDE7] bg-[#EEF2FF] text-[#061E81]',
  ACCEPTED: 'border-[#D9DDE7] bg-[#F4F2ED] text-[#303844]',
        ...
        <section className="storefront-container py-6 sm:py-8 lg:py-10">
          ...
          {isLoading && (
            <div className="grid gap-3">
              {Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="h-40 animate-pulse rounded-[18px] border border-[#D9DDE7] bg-white" />
              ))}
            </div>
          )}
          ...
          {!isLoading && ordersQuery.error && (
            <div className="tn-commerce-card border-[#F2D6D6] p-6 text-[#7A1E1E]">
              ...
              <h2 className="mt-3 text-lg font-bold">{copy.dashboard.orders.errorTitle}</h2>
          ...
          {!isLoading && !ordersQuery.error && orders.length === 0 && (
            <div className="tn-commerce-card p-8 text-center">
              ...
              <h2 className="mt-4 text-xl font-bold text-[#101010]">{copy.dashboard.orders.emptyTitle}</h2>
          ...
          {!isLoading && !ordersQuery.error && orders.length > 0 && (
            <div className="grid gap-4">
              {orders.map((order) => (
                <article key={order.id} className="tn-commerce-card p-4 sm:p-5">
                  <div className="flex flex-col gap-3 border-b border-[#E3E8EF] pb-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.08em] text-[#657080]">{copy.dashboard.orders.orderCode}</p>
                      <h2 className="mt-1 text-xl font-bold text-[#101010]">{order.publicCode}</h2>
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                      ...
                      <span className="text-xl font-bold tabular-nums">{formatGel(order.summary.total)}</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid gap-3">
                    {order.items.map((item) => (
                      <div key={item.id} className="grid grid-cols-[56px_minmax(0,1fr)_auto] items-center gap-3">
                        ...
                        <span className="text-sm font-bold tabular-nums">{formatGel(item.lineTotal)}</span>
                      </div>
                    ))}
                  </div>
                </article>
